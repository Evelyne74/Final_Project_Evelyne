...
# Those files are extracted using a program given as supplementary data
# Because otherwise a connection to Geneva cancer registry is mandatory
...
a_pat_demo<-read_csv(here("data/a_pat_demo.csv") , show_col_types = FALSE) %>% 
  ...
  mutate(gnr_pat_rec = case_when(
    gnr_pat==6999 ~ 9999, # we do not want these unknown gnr code to match with normal gnr codes
...
# Cancer localisation distribution
...
# Merge of total cancer as a tumor category
...
# population by age category and period and gender
...
# the file only contains ref population without any gender
...
# and now we only keep those tumors in the global incidence file
...
```{r Plot the evolution for evolution page, include=FALSE, error=FALSE, warning=FALSE}
#-------------MEN
...
```{r Plot latest period graph for main page, include=FALSE, error=FALSE, warning=FALSE}
...
# Agreagation of number of tumors per gender, period, age category, geographic area
...
# Importation of the 2013-1989 database which is on the same format as 2013-2018
...
# I need to create a dataframe where the reference population has values for male and female so I duplicate the reference population and just add the gender variable for each dataframe, then combine them together
# maybe not usefull....
...
# The town's name has been lost and need to be imported agains
...
final_incidence_rate_by_geo_1989_2021_men<-final_incidence_rate_by_geo %>% 
 ...
 facet_men<-ggplot(data = final_incidence_rate_by_geo_1989_2021_men) +
  ...
  theme_void()+
  labs(fill=str_wrap("Standardised rates",width=20),
       title=str_wrap("Cancer incidence rates between 1989 and 2021- made-up data", width=80))
